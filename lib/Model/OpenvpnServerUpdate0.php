...
/**
 * OpenvpnServerUpdate0 Class Doc Comment
 *
 * @category Class
 * @package  RMoore\Truenas
 * @author   OpenAPI Generator team
 ...
 */
class OpenvpnServerUpdate0 implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'openvpn_server_update_0';
    ...
    public const TOPOLOGY_NET30 = 'NET30';
    public const TOPOLOGY_P2_P = 'P2P';
    public const TOPOLOGY_SUBNET = 'SUBNET';
     ...
     * @return string[]
     */
    public function getTopologyAllowableValues()
    {
        return [
            self::TOPOLOGY_NET30,
            self::TOPOLOGY_P2_P,
            self::TOPOLOGY_SUBNET,
        ];
    }
    ...
    */
    ...
    public function listInvalidProperties()
    {
        ...
        $allowedValues = $this->getTopologyAllowableValues();
        if (!is_null($this->container['topology']) && !in_array($this->container['topology'], $allowedValues, true)) {
            $invalidProperties[] = sprintf(
                "invalid value '%s' for 'topology', must be one of '%s'",
                $this->container['topology'],
                implode("', '", $allowedValues)
            );
        }
     ...
     *
     * @param int|null $server_certificate server_certificate
     *
     * @return self
     */
    public function setServerCertificate($server_certificate)
    {
        ...
        if (is_null($server_certificate)) {
            array_push($this->openAPINullablesSetToNull, 'server_certificate');
        } else {
            $nullablesSetToNull = $this->getOpenAPINullablesSetToNull();
            $index = array_search('server_certificate', $nullablesSetToNull);
            if ($index !== FALSE) {
    ...
    {
        $allowedValues = $this->getTopologyAllowableValues();
        if (!is_null($topology) && !in_array($topology, $allowedValues, true)) {
            throw new \InvalidArgumentException(
                sprintf(
                    "Invalid value '%s' for 'topology', must be one of '%s'",
                    $topology,
        ...
        if (is_null($topology)) {
            array_push($this->openAPINullablesSetToNull, 'topology');
        } else {
            $nullablesSetToNull = $this->getOpenAPINullablesSetToNull();
            $index = array_search('topology', $nullablesSetToNull);
            if ($index !== FALSE) {
                unset($nullablesSetToNull[$index]);
                $this->setOpenAPINullablesSetToNull($nullablesSetToNull);
            }
